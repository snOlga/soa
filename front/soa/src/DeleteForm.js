...
function DeleteForm() {
    ...
    const deleteByWeapon = async (e) => {
        e.preventDefault();
        await fetch("http://localhost:9018/humans/by-weapon" + (deleteAll ? "-all" : "") + "?weapon-type=" + weaponType, {
            method: "DELETE",
    ...
    const deleteByCoolness = async (e) => { 
        e.preventDefault();
        await fetch("http://localhost:9018/humans/less-cool-car" + "?max-coolness=" + coolness, {
            method: "DELETE",
            headers: { "Content-Type": "application/json" }
        });
    };
